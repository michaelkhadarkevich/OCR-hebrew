...
async function renderPdfFile(file) {
  ...
  const response = await fetch("/render_pdf", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name: file.name, data_url: await fileToDataUrl(file), scale: 2.0 }),
  });
  ...
  for (let i = 0; i < result.pages.length; i++) {
    const item = result.pages[i];
    setStatus(`Loading ${file.name}: page ${i + 1} of ${result.pages.length}...`);
    const blob = await (await fetch(item.data_url)).blob();
    const imageFile = new File([blob], item.name, { type: "image/png" });
    rendered.push(await loadImageFile(imageFile));
  }
  return rendered;
}
...
async function loadFiles(files) {
  ...
  try {
    for (const file of files) {
      if (isPdfFile(file)) {
        const pdfPages = await renderPdfFile(file);
        pages.push(...pdfPages);
      } else if (isImageFile(file)) {
        pages.push(await loadImageFile(file));
      }
    }
    currentPageIndex = pages.length ? 0 : -1;
    updatePageSelect();
    render();
    setStatus(pages.length ? `Loaded ${pages.length} page(s).` : "No supported files loaded.");
  } catch (error) {
    console.error(error);
    currentPageIndex = pages.length ? 0 : -1;
    updatePageSelect();
...
pdfInput.addEventListener("change", async () => {
  await loadFiles([...pdfInput.files].filter(isPdfFile));
});
