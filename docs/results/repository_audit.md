...
The regularized run used 12 validation lines sampled from `HarmonitManualTrain`, 1,426 training samples, and stopped at step 2,040. Its historical summary's `final_test_cer` was calculated on `valid`, not the later final set. The report's regularized row correctly uses the separate `test` evaluation: 15/32 exact, CER 4.33%.
...
`evaluate_line_folder.py` previously searched only the top directory for PNGs. The final set is flat, so its published evaluations were unaffected, but `valid` stores images in subdirectories. Evaluating that folder would report no pairs.
